...
?>
	...
	<td>
		...
		<center>
        <legend><font size=5 color=blue><br/><br/>wellcome to population information in<?php echo" $zone zone";?></font></legend>
        </center>
		<form>
		...
		$u="select * from birthtable where birthzone='$zone'";
				 ...
				 if($rowCheck>0)
                 {
                   ...
                   $male="select * from birthtable where  sex='male' and birthzone='$zone'";
                 $view=mysql_query($male,$con);
                    $rowmale = mysql_num_rows($view);
                 if($rowmale>0) 
echo " <tr><td colspan=10><font size=3>number of male=   $rowmale </td></tr></font>";

$female="select * from birthtable where  sex='female' and birthzone='$zone'";
...
echo " <tr><td colspan=10><font size=3>number of female =  $rowfemal </td></tr></font>";
    ...
    echo "No Register birth people  in $zone zone"; 


echo "</table></form>";?>
<table class="data-table" style="margin-left: 15px;margin-right: 15px;margin-top: 15px;">
              <font size="4"color="blue"> click here to view detailed information  </font> <form action="" method="post"> <input type="submit" name="viewdetail" value="view_birth_in_detail"></form>
                   <?php
                   
                 if(isset($_POST['viewdetail']))
     {
		$sql="select * from birthtable where birthzone='$zone'";
	...
	<th>birth kebele</th>
                 ...
                 if(isset($_POST['woreda']))
     {
     	...
     	$u="select * from birthtable where birthworeda='$woreda' and birthstatus='born'";
				 $view=mysql_query($u,$con);
					$rowCheck = mysql_num_rows($view);
				 if($rowCheck>0)
                 
                 	echo"<font color=green font size=5> total number of population in $woreda is $rowCheck </font";
                 	else
                 	echo"<font color=red font size=5>has no population in $woreda</font>";
	}
     	?>
			</div>
			  </td>
