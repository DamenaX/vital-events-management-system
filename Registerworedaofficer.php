...
<?php
     ...
     {
		$woredaofficerid=$_POST['woredaofficerid'];
 ...
 $sql="INSERT INTO woredaofficer VALUES('$woredaofficerid','$ufname','$umname','$ulname','$sex','$mobile','$email','$photo','$woredaid')";
 $result=mysql_query($sql,$con);
  ...
  ?>
	...
	if($result)
	{
              echo "<font color=green><h2>woreda officer successfully registered!!!</h2></font>";
	...
	?>
        <legend>regiser woreda officer employees</legend>
	<form action="" method="post">
<table >
      <tr><td width="140px" height="40px">woredaofficerid</td><td width="140px">
	  ...
	  <input type="email" name="emails" pattern="[A-Za-z@-A-Za-z.]+" placeholder="Enter email" required="" >
		...
		<option value=" ">choose woreda name</option>
		<?php
       while ($row=mysql_fetch_assoc($region)) 
       {
      $woredname=$row["woredaname"];
		?>
		<option value="<?php echo $woredname; ?>"><?php echo $woredname; ?></option>
		<?php
	     }
		?>
		</select>
		...
		<tr><td>
	     <input type='submit' name='register' value='Save' ></td><td>
